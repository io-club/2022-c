...
Bool checkfood(struct TFood *pfood, struct TSnake *psnake);
WINDOW *newlogw();
...
int main() {
  ...
  raw();                /* 禁止行缓冲，可以立即见到结果 */
  noecho();             /* 不在终端上显示控制字符，比如 Ctrl+c */
  keypad(stdscr, true); /* 允许用户在终端使用键盘 */
  curs_set(0); /* 设置光标是否可见，0 不可见，1 可见，2 完全可见 */
  refresh(); /* 将虚拟屏幕上的内容写到显示器上，并刷新 */

  g_level = 1;

  /* #define INITRUNLOG()  logwin = newlogw();
   * 创建 log 窗口，就是调用自定义函数 newlogw() */
  INITRUNLOG();

  /* #define RUNLOG(str)  runlog(logwin, str); runlog 窗口打印一下的游戏提示 */
  RUNLOG("  Press 'q' or 'Q' to quit.");
  RUNLOG("  Press 'w/s/a/d' or 'W/S/A/D' to move the snake.");
  RUNLOG("Info:");

  WINDOW *gwin = newgamew(); /* 创建游戏窗口，具体由自定义函数 newgamew 实现 */
...
int refreshgamew(WINDOW *win, struct TSnake *psnake) {
  ...
  case 0:
    ...
    /* 吃到食物了，ffood 置为 0，以便重新绘制食物。
     * 同时，还要增加蛇身的长度，snakegrowup 函数实现
...
/**
 * stuct TSnake 是一个倒置的首尾相连的链表，head->front 指向 snake 的尾部
 * 如：[a]<-[b]<-[c]<-[d]    a 为 head
 *      |              ^     snake 移动的时候，只用 head 指向 d,
 *      `--------------'     并且修改 d 的 (y,x) 为 snake 头移动到的位置。
 */
void movesnake(struct TSnake *psnake) {
...
void snakegrowup(struct TFood *pfood, struct TSnake *psnake) {
  struct TSnakeNode *pnode =
      (struct TSnakeNode *)malloc(sizeof(struct TSnakeNode));
...
void gameover(WINDOW *win, char *str) {
  mvwprintw(win, (int)(GAMEWIN_YLEN / 2), (GAMEWIN_XLEN / 2 - strlen(str) / 2),
            str);
...
struct TSnake *initsnake() {
  ...
  newsnakenode(
      &newsnakenode(
           &newsnakenode(&newsnakenode(&psnake->head, 4, 50)->front, 4, 53)
                ->front,
           4, 52)
           ->front,
      4, 51)
...
void drawsnakew(WINDOW *win, struct TSnake *psnake) {
  ...
  static int tailx = 0;
  if (taily != 0 && tailx != 0) {
    /* mvwaddch 是把光标移动到指定窗口中 (win) 的指定位置 (taily,tailx)
     * 然后输出字符
     */
...
void cleanline(WINDOW *win, int y, int x) {
  ...
  /* 将光标移动到窗口 win 的 (y,x) 然后打印字符串 EMPTYLINE */
